...
- `Codex-fable-5` — most powerful, newest tier (Fable 5)
- `Codex-opus-4-8` — smartest Opus — **default for the Anthropic provider**
- `Codex-opus-4-7` — previous flagship (Opus 4.7)
- `Codex-opus-4-6` — older flagship (Opus 4.6) — retained: dropping a registry row silently disables Extended Thinking for users still on it (registry drives `reasoningSupport`); drop only once a retired-models concept exists
