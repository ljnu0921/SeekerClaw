...
4. Re-issue the call with the openapi-correct shape (arrays as arrays, string-typed numbers as quoted strings, gateway-named fields) — but **only after confirming with the user that you want to spend more USDC on a retry**. Don't silently re-burn.
