...
- **App update (Store / `adb install -r`):** user memory files (SOUL.md, MEMORY.md, IDENTITY.md, USER.md, daily notes, cron jobs) and any user-created or user-customized skills are preserved verbatim — no impact on prior conversations, agent personality, or scheduled tasks. Bundled default skills (the 22 in `assets/default-skills/`) are version-aware: when the bundled SKILL.md version advances, `ConfigManager.seedSkill()` re-seeds the workspace copy and overwrites support files (e.g., paysh-catalog's `catalog.json` + `services/*.md` re-seed through 1.1.0 → 1.4.0 → … → 1.8.0 this release). This is deliberate and how 2.0.0 ships updated catalog docs to existing devices.
