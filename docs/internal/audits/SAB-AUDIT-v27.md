...
- **`agent_pay returned wrong/empty/error data after settle — gateway rejected the body or params (doc-vs-gateway divergence)`** — symptom patterns covering both POST (HTTP 400/422 with field-shape complaints) and GET (HTTP 400 on bad query params, HTTP 200 with silent garbage data); the three gateway divergence classes (arrays vs strings, string-typed numbers, renamed fields); curl recipes (POST and GET variants) that derive `<base>` from `catalog.json entries[].upstream_ref.service_url`; passthrough exception scoped to GET only (**tripadvisor + wolframalpha** — the two services whose openapi declares zero query params but accepts upstream-style passthrough); explicit note that 2captcha/reducto/perplexity/textbelt/stablecrypto are POST endpoints with openapi-verified body schemas (NOT in the passthrough exception); plus a separate clause that POST 4xx isn't always a shape bug — 2captcha can reject unsupported task types, reducto can reject inaccessible document URLs, etc., diagnosed via the specific error body. Fix steps cover both runtime workaround (re-issue with correct shape) and maintainer fix (rewrite doc + bump SKILL.md version).
