...
- **`agent_pay returned wrong/empty/error data after settle — investigate before retrying`** — symptom patterns covering both POST and GET 4xx/5xx (any `settle_http_error` from x402's `status >= 400` branch — body is NOT surfaced today, follow-up to extend the existing `payment_rejected.diag.bodyHead` pattern tracked separately); the three indistinguishable cause classes (doc-vs-gateway divergence, application-level validation, gateway 5xx/transient); js_eval+`require('https')` recipes (POST and GET variants — `shell_exec` can't pipe to jq and `web_fetch` throws on non-2xx, so neither work for openapi/402 inspection) that derive `<base>` from `catalog.json entries[].upstream_ref.service_url` and substitute any `{param}` path placeholders; passthrough exception scoped to GET only (**tripadvisor + wolframalpha** — the two services whose openapi declares zero query params but accepts upstream-style passthrough); explicit note that 2captcha/reducto/perplexity/textbelt/stablecrypto are POST endpoints with openapi-verified body schemas (NOT in the passthrough exception). Fix steps cover both runtime workaround (re-issue with correct shape, only after explicit user authorization for another paid call) and maintainer fix (rewrite doc + bump SKILL.md version).

- **`agent autonomously paid for trivia / activated paysh-catalog without an explicit pay-intent keyword`** — symptom (USDC burned on a trivia question), diagnosis (BAT-704 OPT-IN regression — list of trigger keywords + common over-match causes), fix (verify SKILL.md version `>= "1.1.0"` for opt-in, verify prompt door is intact via the system prompt's Wallets section, **apologize and explain only — never initiate a refund transfer (agent has no refund tool / no treasury / no entitlement to spend from any other wallet to "make it right"; direct user to project maintainer for any operator-side refund)**, report mismatched trigger phrase).
